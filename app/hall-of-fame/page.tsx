...
export default function HallOfFamePage() {
    ...
    // Sort by +/- (Wins - Losses), then by Win% when equal - Top 35
    const hallOfFame = [...leaderboardData]
        .sort((a, b) => {
            ...
            // Secondary: Win% descending (when +/- is equal)
                    ...
                    <div className="flex flex-col md:flex-row justify-center items-end gap-8 mb-20 relative px-4">
                        ...
                        {second && (
                            <div className="order-2 md:order-1 w-full md:w-72 relative flex flex-col items-center">
                                ...
                                <Link href={`/gvc/${second.id}`} className="block relative bg-[#111] rounded-2xl overflow-hidden border-3 border-gray-400 w-full hover:scale-105 transition-transform duration-300 shadow-[0_0_40px_rgba(192,192,192,0.3)] cursor-pointer">
                                    ...
                                    <div className="p-5 text-center bg-gradient-to-b from-gray-800 to-[#1a1a1a]">
                                        ...
                                        <div className="text-4xl font-display text-gray-300 mb-1">
                                            {second.allTime.wins}-{second.allTime.losses} <span className={`text-2xl ${second.allTime.wins - second.allTime.losses > 0 ? 'text-green-400' : second.allTime.wins - second.allTime.losses < 0 ? 'text-red-400' : 'text-gray-400'}`}>({second.allTime.wins - second.allTime.losses > 0 ? '+' : ''}{second.allTime.wins - second.allTime.losses})</span>
                                        </div>
                        ...
                        <div className="order-1 md:order-2 w-full md:w-96 relative -mt-12 md:-mt-24 z-10 flex flex-col items-center">
                            ...
                            <Link href={`/gvc/${first.id}`} className="block relative bg-[#111] rounded-3xl overflow-hidden border-4 border-gvc-gold w-full hover:scale-105 transition-transform duration-300 shadow-[0_0_80px_rgba(255,204,77,0.5)] cursor-pointer">
                                ...
                                <div className="p-4 text-center bg-gradient-to-b from-gvc-gold via-gvc-gold to-yellow-600">
                                    ...
                                    <div className="text-5xl font-display text-black mb-1 drop-shadow-lg">
                                        {first.allTime.wins}-{first.allTime.losses} <span className="text-3xl">({first.allTime.wins - first.allTime.losses > 0 ? '+' : ''}{first.allTime.wins - first.allTime.losses})</span>
                                    </div>
                        ...
                        {third && (
                            <div className="order-3 w-full md:w-72 relative flex flex-col items-center">
                                ...
                                <Link href={`/gvc/${third.id}`} className="block relative bg-[#111] rounded-2xl overflow-hidden border-3 border-[#CD7F32] w-full hover:scale-105 transition-transform duration-300 shadow-[0_0_40px_rgba(205,127,50,0.3)] cursor-pointer">
                                    ...
                                    <div className="p-5 text-center bg-gradient-to-b from-[#8B4513] to-[#1a1a1a]">
                                        ...
                                        <div className="text-4xl font-display text-[#CD7F32] mb-1">
                                            {third.allTime.wins}-{third.allTime.losses} <span className={`text-2xl ${third.allTime.wins - third.allTime.losses > 0 ? 'text-green-400' : third.allTime.wins - third.allTime.losses < 0 ? 'text-red-400' : 'text-gray-400'}`}>({third.allTime.wins - third.allTime.losses > 0 ? '+' : ''}{third.allTime.wins - third.allTime.losses})</span>
                                        </div>
                                        <div className="text-lg text-white font-mono mb-3">
                    ...
                    {runnersUp.map((char, index) => {
                        ...
                        return (
                            <div key={char.id} className={`relative group opacity-90 hover:opacity-100 transition-opacity ${is69 ? 'scale-[1.02]' : ''}`}>
                                <Link href={`/gvc/${char.id}`} className={`block relative bg-[#111] rounded-xl overflow-hidden border transition-colors cursor-pointer ${is69
                                    ...
                                    <div className="p-4 text-center">
                                        ...
                                        <div className={`text-3xl font-display mb-1 ${is69 ? 'text-pink-500' : 'text-white'}`}>
                                            {char.allTime.wins}-{char.allTime.losses} <span className={`text-xl ${char.allTime.wins - char.allTime.losses > 0 ? 'text-green-400' : char.allTime.wins - char.allTime.losses < 0 ? 'text-red-400' : 'text-gray-400'}`}>({char.allTime.wins - char.allTime.losses > 0 ? '+' : ''}{char.allTime.wins - char.allTime.losses})</span>
                                        </div>
