...
import BuyVotesModal from '@/components/BuyVotesModal';
...
export default function DuosPage() {
    ...
    const fetchMatchup = async (addToQueue = false, walletOverride?: string, skipPenalty = false) => {
        ...
        try {
            ...
            const wallet = walletOverride || address;
            if (wallet) params.append('wallet', wallet);
            if (skipPenalty || addToQueue) params.append('skipPenalty', 'true');

            // Force query string locally to ensure we catch addToQueue logic
            const queryString = params.toString() ? `?${params.toString()}` : '';

            const res = await fetch(`/api/duos/matchup${queryString}`, {
    ...
    const fetchVotes = async () => {
        try {
            ...
            const res = await fetch(`/api/duos/vote${walletParam}`, {
                headers: { 'x-duos-session-id': getSessionId() }
            });
            const data = await res.json();
    ...
    // Re-fetch when wallet connects so bonus votes are included in the limit
    useEffect(() => {
        if (address) {
            fetchMatchup(false, undefined, true); // Skip penalty on wallet connect refresh
            fetchVotes();
        }
    }, [address]);
    ...
    const handleVote = useCallback(async (winnerId: string, loserId: string) => {
        if (voting || remainingVotes === null || remainingVotes <= 0 || isTransitioning) return;
        setVoting(true);
        ...
        try {
            const res = await fetch('/api/duos/vote', {
                ...
                body: JSON.stringify({ winnerId, loserId, walletAddress: address || undefined })
            });
            const data = await res.json();
            if (data.success) {
                setRemainingVotes(data.remainingVotes);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (!matchup || remainingVotes === null || remainingVotes <= 0 || showLeaderboard || isTransitioning) return;
    ...
    return (
        <>
            ...
            <BuyVotesModal
                ...
                onSuccess={() => {
                    fetchVotes();
                    fetchMatchup(false, address, true); // Skip penalty on purchase refresh
                }}
            />
        </>
    );
