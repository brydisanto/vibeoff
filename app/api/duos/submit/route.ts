...
export async function POST(request: Request) {
    try {
        ...
        // Check if user owns both GVCs
        const [owner1, owner2] = await Promise.all([
            kv.hget(`stats:alltime:${gvc1Id}`, 'owner'),
            kv.hget(`stats:alltime:${gvc2Id}`, 'owner')
        ]);

        const owner1Normalized = typeof owner1 === 'string' ? owner1.toLowerCase() : null;
        const owner2Normalized = typeof owner2 === 'string' ? owner2.toLowerCase() : null;

        if (owner1Normalized !== normalizedWallet) {
            return NextResponse.json({ 
                error: `You don't own GVC #${gvc1Id}` 
            }, { status: 403 });
        }

        if (owner2Normalized !== normalizedWallet) {
            return NextResponse.json({ 
                error: `You don't own GVC #${gvc2Id}` 
            }, { status: 403 });
        }
